...
function answerQuestion($questionUUID, $answers)
{
    ...
    if (empty($answers)) {
        $missatges[] = array(
            'type' => 'warning',
            'msg' => '<strong>Atenção</strong>: Você não selecionou nenhuma resposta, tente novamente.'
        );
    ...
    $missatges[] = array(
        ...
        'msg' => sprintf(
            ...
            . "Voce ganhou <strong>%d pontos</strong>.",
            $points
        )
    );

    $oldLevel = getUserLevelById($userId);
    doSilentAddExperience($userId, $points, 'Responda a pergunta: '. $question['name']);
    ...
    if ($oldLevel != $newLevel) {
        ...
        $missatges[] = array(
            'type' => "success",
            'msg' => sprintf(
                "<strong>Boa!</strong> Você acabou de subir de nível! Agora você é um <strong>'%s'</strong>.",
                $levelName
            )
        );
    }
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ( intval($actionResult) === intval($row['badge_id'])) {
                ...
                $missatges[] = array(
                    'type' => "success",
                    'msg' => sprintf(
                        "<strong>Boa!</strong> Você acabou de conquistar a Badge <strong>'%s'</strong>.",
                        $badgeName
                    )
                );
            }
...
function printQuestionHeader($a = 'list')
{
    ...
    <ul class="nav nav-tabs">
        <li<?= ('list' === $a) ? ' class="active"' : ''; ?>>
            <a href="<?= $_SERVER['PHP_SELF']; ?>?a=list">Resposta pendente</a>
        </li>
        <li<?= ('historic' === $a) ? ' class="active"' : ''; ?>>
            <a href="<?= $_SERVER['PHP_SELF']; ?>?a=historic">Revisar perguntas</a>
        </li>
...
function printQuestionList()
{
    ...
    if (is_null($questions)) {
        ...
        $htmlCode[] = getHTMLMessages(
            array(
                'type' => "info",
                'msg' => "<strong>Parabéns</strong>. "
                . "Você não tem perguntas a fazer. Você ainda pode descobrir como continuar participando!"
            )
        );
    } else {
...
function printHistoricQuestionList()
{
    ...
    if (is_null($questions)) {
        ...
        $htmlCode[] = getHTMLMessages(
            array(
                'type' => "info",
                'msg' => "Não há mais perguntas. Espere a próxima track"
            )
        );
    } else {
...
function viewQuestionByUUID($questionUUID, $msg = array())
{
    ...
    if (!$answered) {
        echo '<div class="alert alert-warning">'
            . '<p>Você não respondeu essa pergunta.</p>'
            . '</div>';
    } else {
        if (!empty($question['solution'])) {
            echo '<div class="alert alert-success">'
            . '<p><strong>A resposta correta é: </strong></p><p>'
    ...
    <?php
    if ($answered) {
        echo '<div class="alert alert-info"><p>Você respondeu essa pergunta em '
            . $responses['last_time'] . ' e ganhou <strong>'
            . $responses['amount'] .' pontos</strong>.</p></div>';
    }
    ?>
        </div>
    </div>
...
function getHTMLQuestionChoices($choices, $answers, $answered)
{
    ...
    foreach ($choices as $choice) {
        ...
        if ($choice['points'] > 0) {
            $htmlCode[] = '<span class="label label-success pull-right">' . $choice['points'] . ' pontos</span>';
        } else {
            $htmlCode[] = '<span class="label label-danger pull-right">' . $choice['points'] . ' pontos</span>';
        }
        if (in_array($choice['id'], explode(',', $answers))) {
            $htmlCode[] = '<span class="label label-info">Sua resposta</span>';
        }
        $htmlCode[] = $choice['choice'];
        $htmlCode[] = '</li>';
    }

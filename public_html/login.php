...
function doRegister($data = array())
{
    ...
    if (!filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
        $missatges[] = array('type' => "error", 'msg' => "E-mail incorreto.");
    ...
    if (getUserExists($data['username'])) {
        $missatges[] = array(
            'type' => "info",
            'msg' => "O usuário '<strong>". $data['username'] ."</strong>' já existe."
        );
    ...
    switch ($CONFIG['authentication']['type']) {
        case 'LDAP':
            if (false === getLDAPAuth($data['username'], $data['password'])) {
                ...
                $missatges[] = array(
                    'type' => "error",
                    'msg' => "Não foi possível verificar suas credenciais LDAP. Revise-os"
                );
                printRegisterForm($missatges);
                return false;
            }
            break;
        default:
            die("TODO: registro para usuários não LDAP ainda não implementados!");
    }
    ...
    if (0 === $userId) {
        $missatges[] = array('type' => "error", 'msg' => "Falha ao criar usuário.");
        printRegisterForm($missatges);
        ...
        $missatges[] = array(
            'type' => "success",
            'msg' => "O usuário '<strong>". $data['username'] ."</strong>' foi criado."
        );
        printLoginForm($data['username'], $missatges);
